...
do
	...
	do
		...
		RGID=${NODIR%%_R1*}
		echo $R1 $R2 $RGID
		bowtie2 -x ${REFERENCE} -1 $R1 -2 $R2 -p 39 --local --rg-id $RGID --rg SM:$RGID --rg LB:LIBRARY --rg PL:ILLUMINA 2> ../../logs/${RGID}_bowtie2_local.log | samtools view -bS - > ../../bams/WGS/${TAG}_${NODIR%%.fastq.gz}_BOWTIE2LOC.bam
