...
docker run -v /media/array/callers_proj/raw_data:/data broadinstitute/gatk:latest gatk --java-options "-Xmx8g" BaseRecalibrator -R /data/$REFERENCE -I /data/bams/$i --known-sites /data/bundle/$DBSNP --known-sites /data/bundle/$MILLS_INDELS --known-sites /data/bundle/$GENOMES -O /data/${i%%.dedup.bam}/${i%%.dedup.bam}.recal.table -L /data/$CALLINGREGIONS
docker run -v /media/array/callers_proj/raw_data:/data broadinstitute/gatk:latest gatk --java-options "-Xmx8g" ApplyBQSR -R /data/$REFERENCE -I /data/bams/$i -bqsr /data/${i%%.dedup.bam}/${i%%.dedup.bam}.recal.table -O /data/${i%%.dedup.bam}/${i%%.dedup.bam}.recal.bam -L /data/$CALLINGREGIONS
