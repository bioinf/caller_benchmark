...
do
	...
	do
		docker run -v /media/array/callers_proj/raw_data/:/data google/deepvariant:latest /opt/deepvariant/bin/run_deepvariant --model_type=WES --ref=/data/$REFERENCE --reads=/data/bams/$i --regions=/data/gencode.v19.revamp.bed --output_vcf=/data/${i%%.dedup.bam}/${i%%.dedup.bam}_DV_STANDART.vcf --output_gvcf=/data/${i%%.dedup.bam}/${i%%.dedup.bam}.DV.g.vcf --num_shards=6 &
	done
	wait
done
