...
do
	...
	echo "Aligning $TAG"
	bowtie2 -x ${REFERENCE} -1 ${TMPTAG}.R1.fastq.gz -2 ${TMPTAG}.R2.fastq.gz -p 36 --rg-id $SMP --rg SM:$SMP --rg LB:LIBRARY --rg PL:ILLUMINA 2> ./logs/${SMP}_bowtie2.log | samtools view -bS - > ./bams/${TAG}_BOWTIE2.bam 
done
